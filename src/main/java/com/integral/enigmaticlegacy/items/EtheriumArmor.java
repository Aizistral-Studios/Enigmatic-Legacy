...
public class EtheriumArmor extends ArmorItem implements IPerhaps {
	...
	public static boolean hasShield(@Nonnull PlayerEntity player) {
		if (player != null)
			if (EtheriumArmor.hasFullSet(player) && player.getHealth() / player.getMaxHealth() <= ConfigHandler.ETHERIUM_ARMOR_SHIELD_THRESHOLD.getValue().asMultiplier(false))
				return true;
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> list, ITooltipFlag flagIn) {
		...
		if (EtheriumArmor.hasFullSet(Minecraft.getInstance().player)) {
