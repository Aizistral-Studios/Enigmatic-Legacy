	...
	*/
	...
	public void tick() {
			...
			if (this.target != null && this.target.isAlive()) {
				...
				*/
				...
				if (distance < 6D && distance != 0)
					this.setMotion(new Vec3d(formerMotion.x + (motionVec.x * 2D / distance), formerMotion.y + (motionVec.y * 2D / distance), formerMotion.z + (motionVec.z * 2D / distance)).normalize().mul(1.4D, 1.4D, 1.4D));
		...
		if (this.ticksExisted < 10) {
			Vector3 res = AOEMiningHelper.calcRayTrace(this.world, (PlayerEntity) this.shootingEntity, FluidMode.NONE, 128);
			...
			this.initMotion(this.shootingEntity, res.x - this.getPosX(), res.y - this.getPosY(), res.z - this.getPosZ(), 1.5F);
		...
		if (!this.world.isRemote) {
			...
			if (result.getType() == RayTraceResult.Type.ENTITY) {
				...
				if (this.shootingEntity != null) {
					if (entity.attackEntityFrom(DamageSource.causeIndirectMagicDamage(this, this.shootingEntity), this.isSkullInvulnerable() ? 24.0F : 8.0F)) {
						if (entity.isAlive()) {
			...
			if (!net.minecraftforge.event.ForgeEventFactory.onExplosionStart(this.world, explosion)) {
				...
				for(ServerPlayerEntity serverplayerentity : this.world.getServer().getWorld(this.world.getDimension().getType()).getPlayers()) {
			...
			EnigmaticLegacy.packetInstance.send(PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint(this.getPosX(), this.getPosY(), this.getPosZ(), 64, this.dimension)), new PacketWitherParticles(this.getPosX(), this.getPosY() + (this.getHeight() / 2), this.getPosZ(), this.isSkullInvulnerable() ? 20 : 16, false));
