...
public class QuoteHandler {
	...
	private void drawQuote(PoseStack stack, Window window) {
		if (this.getPlayTime() > this.currentQuote.getSubtitles().getDuration()) {
		...
		for (String line : text) {
			font.drawShadow(stack, line, window.getGuiScaledWidth() / 2 - font.width(line) / 2, height + (counter * (font.lineHeight + 2)), ChatFormatting.YELLOW.getColor());
			counter++;
		}
